...
import mod.patrigan.slimier_slimes.client.entity.render.layers.ShroomSlimeMushroomLayer;
...
public class ShroomSlimeRenderer extends MobRenderer<ShroomSlimeEntity, SlimeModel<ShroomSlimeEntity>> {
    ...
    public ShroomSlimeRenderer(EntityRendererProvider.Context context) {
        super(context, new SlimeModel<>(context.bakeLayer(ModelLayers.SLIME)), 0.7f);
        this.addLayer(new SlimeOuterLayer<>(this, context.getModelSet()));
        this.addLayer(new ShroomSlimeMushroomLayer<>(this, context.getModelSet()));
    }

    @Override
    public void render(ShroomSlimeEntity entityIn, float entityYaw, float partialTicks, PoseStack matrixStackIn, MultiBufferSource bufferIn, int packedLightIn) {
        this.shadowRadius = 0.25F * (float)entityIn.getSize();
        super.render(entityIn, entityYaw, partialTicks, matrixStackIn, bufferIn, packedLightIn);
    }

    @Override
    protected void scale(ShroomSlimeEntity entitylivingbaseIn, PoseStack matrixStackIn, float partialTickTime) {
        float f = 0.999F;
