...
public class CastingAttackGoal extends Goal {
    ...
    public void tick() {
        ...
        boolean flag = this.slimeEntity.getSensing().hasLineOfSight(this.attackTarget);
        if (!flag) {
            ++this.notSeeTime;
        } else {
            this.notSeeTime = 0;
        ...
        moveHelper.setDirection(this.slimeEntity.getYRot(), this.slimeEntity.isDealsDamage());
