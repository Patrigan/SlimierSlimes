...
public class PillagerSlimeLab extends StructureFeature<NoneFeatureConfiguration> {

    public static final String STRUCTURE_ID = "pillager_slime_lab";

    public PillagerSlimeLab(Codec<NoneFeatureConfiguration> codec) {
        super(codec);
    }

    @Override
    public StructureStartFactory<NoneFeatureConfiguration> getStartFactory() {
        return PillagerSlimeLab.Start::new;
    }
    ...
    private static final List<MobSpawnSettings.SpawnerData> STRUCTURE_MONSTERS = ImmutableList.of(
            ...
            new MobSpawnSettings.SpawnerData(EntityType.EVOKER, 25, 4, 9),
            new MobSpawnSettings.SpawnerData(EntityType.ILLUSIONER, 5, 4, 9),
    ...
    protected boolean isFeatureChunk(ChunkGenerator chunkGenerator, BiomeSource biomeSource, long seed, WorldgenRandom chunkRandom, ChunkPos chunkPos, Biome biome, ChunkPos chunkPos1, NoneFeatureConfiguration featureConfig, LevelHeightAccessor levelHeightAccessor) {
        StructureFeatureConfiguration structureseparationsettings = chunkGenerator.getSettings().getConfig(StructureFeature.VILLAGE);
        if (structureseparationsettings == null) {
            return true;
        } else {
            for(int i = chunkPos.x - 10; i <= chunkPos.x + 10; ++i) {
                for(int j = chunkPos.z - 10; j <= chunkPos.z + 10; ++j) {
                    ChunkPos chunkpos = StructureFeature.VILLAGE.getPotentialFeatureChunk(structureseparationsettings, seed, chunkRandom, i, j);
