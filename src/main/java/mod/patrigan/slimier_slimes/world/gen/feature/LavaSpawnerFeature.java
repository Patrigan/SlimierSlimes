...
public class LavaSpawnerFeature extends Feature<LavaSpawnerConfig> {
    public LavaSpawnerFeature(Codec<LavaSpawnerConfig> codec) {
        super(codec);
    }

    @Override
    public boolean place(FeaturePlaceContext<LavaSpawnerConfig> featurePlaceContext) {
        Random rand = featurePlaceContext.random();
        BlockPos pos = featurePlaceContext.origin();
        LavaSpawnerConfig config = featurePlaceContext.config();
        WorldGenLevel reader = featurePlaceContext.level();
        float f = rand.nextFloat() * (float)Math.PI;
        float f1 = (float)config.size / 8.0F;
        int i = (int) Math.ceil(((float)config.size / 16.0F * 2.0F + 1.0F) / 2.0F);
        ...
        int k = pos.getX() - (int) Math.ceil(f1) - i;
        int l = pos.getY() - 2 - i;
        int i1 = pos.getZ() - (int) Math.ceil(f1) - i;
        int j1 = 2 * ((int) Math.ceil(f1) + i);
        int k1 = 2 * (2 + i);
    ...
    protected boolean doPlace(LevelAccessor worldIn, Random random, LavaSpawnerConfig config, double p_207803_4_, double p_207803_6_, double p_207803_8_, double p_207803_10_, double p_207803_12_, double p_207803_14_, int p_207803_16_, int p_207803_17_, int p_207803_18_, int p_207803_19_, int p_207803_20_) {
        int i = 0;
        ...
        for(int j3 = 0; j3 < j; ++j3) {
            ...
            if (d11 >= 0.0D) {
                ...
                int l = Math.max((int) Math.floor(d1 - d11), p_207803_16_);
                int l3 = Math.max((int) Math.floor(d3 - d11), p_207803_17_);
                int i1 = Math.max((int) Math.floor(d5 - d11), p_207803_18_);
                int j1 = Math.max((int) Math.floor(d1 + d11), l);
                int k1 = Math.max((int) Math.floor(d3 + d11), l3);
                int l1 = Math.max((int) Math.floor(d5 + d11), i1);
                                ...
                                for(int k2 = i1; k2 <= l1; ++k2) {
                                    ...
                                    if (d8 * d8 + d9 * d9 + d10 * d10 < 1.0D) {
                                        ...
                                        if (!bitset.get(l2)) {
                                            ...
                                            if (config.target.test(worldIn.getBlockState(blockpos$mutable), random) && worldIn.getBlockState(blockpos$mutable.offset(0, 1, 0)).getBlock().equals(LAVA)) {
