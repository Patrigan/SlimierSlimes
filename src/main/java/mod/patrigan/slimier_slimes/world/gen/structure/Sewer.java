...
import static mod.patrigan.slimier_slimes.world.gen.structure.WaterFlowFixPostPlacementProcessor.WATER_FLOW_FIX;

@Mod.EventBusSubscriber(modid = SlimierSlimes.MOD_ID, bus = Mod.EventBusSubscriber.Bus.FORGE)
public class Sewer extends StructureFeature<JigsawConfiguration> {

    public static final String STRUCTURE_ID = "sewer";

    public Sewer(Codec<JigsawConfiguration> codec) {
        super(codec, (context) -> {
                    // Check if the spot is valid for structure gen. If false, return nothing to signal to the game to skip this spawn attempt.
                    if (!Sewer.isFeatureChunk(context)) {
                        return Optional.empty();
                    }
                    // Create the pieces layout of the structure and give it to
                    else {
                        return Sewer.createPiecesGenerator(context);
                    }
                },
    ...
    private static final Lazy<List<MobSpawnSettings.SpawnerData>> STRUCTURE_MONSTERS = Lazy.of(() -> List.of(
            new MobSpawnSettings.SpawnerData(COMMON_SLIME.get(), 50, 1, 1),
            new MobSpawnSettings.SpawnerData(BROWN_GOO_SLIME.get(), 50, 1, 1),
            new MobSpawnSettings.SpawnerData(EntityType.ZOMBIE, 20, 4, 4),
            new MobSpawnSettings.SpawnerData(EntityType.SKELETON, 20, 4, 4),
            new MobSpawnSettings.SpawnerData(SHROOM_SLIME.get(), 20, 4, 4),
            new MobSpawnSettings.SpawnerData(DIAMOND_SLIME.get(), 20, 1, 1)
    ));
    ...
    public static void setupStructureSpawns(final StructureSpawnListGatherEvent event) {
        if(event.getStructure() == ModStructures.SEWER.get()) {
...
//    int heightY = Math.min(65, chunkGenerator.getBaseHeight(x, z, Heightmap.Types.WORLD_SURFACE_WG, levelHeightAccessor));
