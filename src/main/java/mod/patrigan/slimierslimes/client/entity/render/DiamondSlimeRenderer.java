...
public class DiamondSlimeRenderer extends MobRenderer<DiamondSlimeEntity, SlimeModel<DiamondSlimeEntity>> {
    ...
    public DiamondSlimeRenderer(EntityRendererManager renderManagerIn) {
        super(renderManagerIn, new SlimeModel<>(16), 0.7f);
        this.addLayer(new SlimeGelLayer<>(this));
    }

    @Override
    public void render(DiamondSlimeEntity entityIn, float entityYaw, float partialTicks, MatrixStack matrixStackIn, IRenderTypeBuffer bufferIn, int packedLightIn) {
        this.shadowSize = 0.25F * (float)entityIn.getSlimeSize();
        super.render(entityIn, entityYaw, partialTicks, matrixStackIn, bufferIn, packedLightIn);
    }

    @Override
    protected void preRenderCallback(DiamondSlimeEntity entitylivingbaseIn, MatrixStack matrixStackIn, float partialTickTime) {
        float f = 0.999F;
    ...
    @Override
    public ResourceLocation getEntityTexture(DiamondSlimeEntity entity) {
        return TEXTURE;
    }
}
